...
RegisterNetEvent('apartments:client:UpdateApartment', function()
    ...
    -- Sunucuya apartmanın mevcut olup olmadığını kontrol etmesi için bir istek gönder
	Wait(300)
    TriggerServerEvent("apartments:server:CheckApartment", apartmentType, apartmentLabel)
end)

RegisterNetEvent('apartments:client:ApartmentStatus', function(hasApartment, apartmentType, apartmentLabel)
    Wait(300)
	if hasApartment then
        -- Apartman varsa güncelle
        TriggerServerEvent("apartments:server:UpdateApartment", apartmentType, apartmentLabel)    
        IsOwned = true
        DeleteApartmentsEntranceTargets()
        DeleteInApartmentTargets()
    else
        -- Apartman yoksa oluştur
        TriggerServerEvent("apartments:server:CreateApartment", apartmentType, apartmentLabel)
		DeleteApartmentsEntranceTargets()
        DeleteInApartmentTargets()
    end
end)
