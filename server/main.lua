...
RegisterNetEvent('apartments:server:CheckApartment', function(apartmentType, apartmentLabel)
    local src = source
    local Player = QBCore.Functions.GetPlayer(src)
	local result = MySQL.query.await('SELECT * FROM apartments WHERE citizenid = ?', { Player.PlayerData.citizenid })
    local hasApartment = (result[1] ~= nil)	
	
    if Player.PlayerData.money['cash'] >= 5000 then
        local success = Player.Functions.RemoveMoney('cash', 5000, "Taşınma ücreti")     
        if success then
			Wait(300)
            TriggerClientEvent('apartments:client:ApartmentStatus', src, hasApartment, apartmentType, apartmentLabel)
        else
            TriggerClientEvent('QBCore:Notify', src, "Para çekme işlemi başarısız oldu.")
        end
    else
        TriggerClientEvent('QBCore:Notify', src, "Yeterli paranız yok.")
    end	
end)
